...
This document bounds the **first minimal real PTY** work in `ana_term`: open and close a POSIX pseudoterminal pair on Linux only, behind the existing **guarded transport** opt-in. No interactive workload, no child process attachment, no ConPTY.
