...
This document defines module ownership for `ana_term`. Each area has a single owner; cross-cutting concerns go through explicit seams (types, interfaces, and file boundaries), not by reaching into another module’s internals.
